...
class Dashboard_model extends Ci_Model{
  ...
  function get_maxd($pl){
        ...
        {
            $query = "SELECT nama_shift AS kelas, COUNT(*) AS total_santrikelas FROM tb_siswa
                        GROUP BY nama_shift";
    ...
    function get_max2($in){
      ...
      function get_max3($in){
            $select = array('a.id_shift,count(b.id_karyawan) as total_karyawan');
            $this->db->select($select);
            $this->db->from('karyawan as b , shift as a');
            $this->db->where('b.id_shift=a.id_shift');
            $this->db->group_by('b.id_shift');
            $this->db->order_by('total_karyawan desc, b.id_karyawan');
            return $this->db->get();
    }
