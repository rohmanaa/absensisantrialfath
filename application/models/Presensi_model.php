...
function get_by_tidakabsen()
{
    $today = date('Y-m-d'); // Get the current date

    $tidak = "SELECT id_karyawan, nama_karyawan 
              FROM karyawan 
              WHERE NOT EXISTS (
                  SELECT id_karyawan 
                  FROM presensi 
                  WHERE karyawan.id_karyawan = presensi.id_karyawan 
                  AND presensi.tgl = '$today'
              )";
    
    return $this->db->query($tidak)->result();
}
