...
class Presensi extends CI_Controller
{
    ...
    {
        $user = $this->user;
        $Presensi= $this->Presensi_model->get_by_tidakabsen();
        $data = array(
            'button' => 'INPUT',
            'action' => site_url('presensi/submit'),
        ...
        $this->template->load('template/template', 'presensi/presensi_tidakabsen', $data);
        $this->load->view('template/datatables');
    }
    ...
    if (!$this->ion_auth->is_admin()) {
     ...
     foreach ($Presensi as $Presensi) {
               ...
               $data = array(
                ...
                'tgl' => $tgll,
                'id_khd' => 4,
                'ket' => 'Tidak Absen dan Tanpa Keterangan',
                'id_status' => 4,
            );
            $this->Presensi_model->insert($data);
     }
